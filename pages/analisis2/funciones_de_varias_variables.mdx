...
> Una función $f$ de $n$ variables es una regla que asigna a cada $n$-tupla de números reales $\overline{x} = (x_1, x_2, \ldots, x_n)$ un único número real $f(\overline{x}) = f(x_1, x_2, \ldots, x_n)$.

Podemos definir su dominio como:

```math 
Dom\;f = \{ \overline{x} \in \mathbb{R}^n \mid f(\overline{x}) \text{ está definida} \}
```

La imagen como: 

```math
Im\;f = \{ y \in \mathbb{R} \mid \exists \overline{x} \in Dom\;f | y = f(\overline{x}) \}
```

Su gráfica se define como: 

```math
G(f) = \{ (\overline{x}, f(\overline{x})) \in \mathbb{R}^{n+1} \mid \overline{x} \in Dom\;f \}
```
...
> Dado $r > 0$ y $\overline{a} \in \mathbb{R}^n$, llamamos bola abierta de centro $\overline{a}$ y radio $r$ al conjunto $B = \{ \overline{x} \in \mathbb{R}^n , ||\overline{x} - \overline{a}|| < r \}$

- Si $n = 1$, $B(\overline{a}, r)$ es intervalo abierto centrado en $\overline{a}$ y de radio $r$).

- Si $n = 2$, $B(\overline{a}, r)$ es un disco abierto centrado en $\overline{a}$ y de radio $r$.

- Si $n = 3$, $B(\overline{a}, r)$ es una bola centrada en $\overline{a}$ y de radio $r$.

### Límite 

> Sea $\overline{a} \in \mathbb{R}^n$ y $f: Dom\;f \subset \mathbb{R}^n \rightarrow \mathbb{R}$, que incluye puntos arbitrarios cercanos a $\overline{a}$ decimos: 

```math
\lim_{\overline{x} \to \overline{a}} f(\overline{x}) = L
```

### Continuidad 

> Sea $f : Dom\;f \subseteq \mathbb{R}^n \rightarrow \mathbb{R}$ y $\overline{a} \in \mathbb{R}^n$, decimos que $f$ es continua en $\overline{a}$ si:

```math
\overline{a} \in Dom\;f \land \lim_{\overline{x} \to \overline{a}} f(\overline{x}) = f(\overline{a})
```

Entonces, $f$ es continua si $f$ es continua en todo punto de su dominio. Valen propiedades similares a funciones continuas de $R$ en $R$. 
...
**Si las derivadas parciales $f_{x_j}$ son continuas en $\overline{a}$, $f$ es continua en $\overline{a}$.**
...
Sea $f : Dom \; f \subseteq \mathbb{R}^n \rightarrow \mathbb{R}, \; \overline{a} \in Dom \; f$ y tal que $\frac{\partial f}{\partial x_1}, \; \ldots, \; \frac{\partial f}{\partial x_n}$ existen y son continuas en $B(\overline{a}, r)$\
para algún $r > 0$

Para $1 \leq i \leq n$ y un intervalo $I \subseteq R$, sean $x_i : I \rightarrow \mathbb{R}$ funciones derivables $\forall t \in I$, la función $g(t) = f(x_1(t), \ldots, x_n(t))$ es derivable en $t \in I$ y:
...
> Decimos que la derivada direccional de $f$ en el punto $\overline{a}$ en la dirección de $\overline{u}$ (siendo $||\overline{u}|| = 1$) es:

```math
D_{\overline{u}} f(\overline{a}) = \displaystyle{\lim_{h \to 0}} \; \frac{f(\overline{a_1} + hu_1, \ldots, a_n + hu_n) - f(a_1, \ldots, a_n)}{h}
```

El gradiente de $f$ en $\overline{a}$ es el vector:

```math
\nabla f(\overline{a}) = \left( \frac{\partial f}{\partial x_1} (\overline{a}), \ldots, \frac{\partial f}{\partial x_n} (\overline{a}) \right)
```
