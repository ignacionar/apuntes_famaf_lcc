...
\begin{array}{ll}
  ...
  \\
  \equiv \{ \textnormal{ Definición de } as \; \} \\
  \\
  \bigl \langle \; \underline{\exists as, bs : [] = as ⧺ bs : prod.[] = 8} \; \bigr \rangle \\
  \\
  \equiv \{ \textnormal{ Evalúo el Término } \} \\
  \\
  \underline{prod.[]} = 8 \\
  \\
...
\begin{array}{ll}
  ...
  \\
  \equiv \{ \textnormal{ Posibles valores de } as \; \} \\
  \\
...
Derivando la asignación, llegamos a un análisis por casos, por lo que podemos usar $\mathbf{if}$ para determinar el programa final:
